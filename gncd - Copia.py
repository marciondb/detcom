...
class Main():
    ...
    GNX = nx.Graph()  # let's create the graph first
    nodesArray = []
    dictWeightEdges = {}
    ...
    def run(self, listGraph, fileWithCommunitiesDetects, nameOfNetwork, numberOfGraph, dictLabel):
        ...
        snap.DrawGViz(self.GraphDet, snap.gvlSfdp,
                      ...
                      "Grafo{} {} com as comunidades detectadas por GirvanNewman".format(numberOfGraph,
                                                                                         nameOfNetwork), H)
        # print "A modularidade da Rede foi? %f" % modularity

        density = self.avgDensity(self.GraphDet, arryToMni)

        result = {}
        result[0] = modularity
        result[1] = self.nodesArray
        result[2] = self.makeVectorToNni(arryToMni, self.nodesArray)
        result[3] = density
        result[4] = arryToMni
